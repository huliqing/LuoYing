...
import name.huliqing.fighter.ui.UISound;
...
//        }
        ...
        // 按下事件应该直接执行
        if (isPressed) {
            clickPressTime = Common.getGameTime();
            lastCursorPosition.set(Common.getCursorPosition());
            pressUI = ui;
...
//        Logger.get(UIClickManager.class).log(Level.INFO, "click time={0}", (Common.getCurrentTime() - clickPressTime));
        if (Common.getGameTime() - clickPressTime > clickPressTimeLimit) {
            return;
        }
        
        // 3.如果鼠标放开时的位置偏移过大，则忽略点击
        Vector2f cursorPosition = Common.getCursorPosition();
        ...
        if (releaseUI == ui && Common.getGameTime() - clickReleaseTime <= dbclickLimit) {
        ...
        if (!ui.hasDBClickEvent()) {
            ...
            releaseUI = ui;
            clickReleaseTime = Common.getGameTime();
        }
    ...
    void update(float tpf) {
        // 延迟执行的单击事件
        if (releaseUI != null 
                && Common.getGameTime()- clickReleaseTime > dbclickLimit) {
