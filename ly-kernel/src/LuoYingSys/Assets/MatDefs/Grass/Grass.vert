...
void main(){
   ...
   vec3 modelSpaceNorm = inNormal;
    ...
    #ifdef SWAYING
		float angle = (g_Time + modelSpacePos.x*m_SwayData.y) * m_SwayData.x;
		angle = 0.1*inTexCoord.y*sin(angle);
		modelSpacePos.xz += m_Wind*angle;
		modelSpaceNorm.xz += m_Wind*angle;
	#endif
	
	float fade = 1.0; 
    #ifdef FADE_ENABLED
	    vec4 wPosition = g_WorldMatrix*modelSpacePos;
		float dist = distance(g_CameraPosition.xz, wPosition.xz);
		fade = clamp((m_FadeEnd - dist)/(m_FadeRange),0.0,1.0);
	#endif
