 ...
 * @author huliqing
 */
public class ParticleEffectDataLoader extends EffectDataLoader{

    @Override
    public void load(Proto proto, EffectData store) {
        super.load(proto, store);
        
        String emitter = proto.getAsString("emitter");
        if (emitter != null) {
            EmitterData ed = Loader.loadData(emitter);
            store.setAttribute("emitter", ed);
        }
    }
