...
public interface SkillPlayListener {
     ...
     * @param source 源角色，即要执行技能的角色。
     * @param skill 指定要执行的技能
     * @return 
     */
    boolean onSkillHookCheck(Actor source, Skill skill);
    
    /**
     * 当角色开始执行某个技能时该方法被调用，这个调用发生在技能
     * 开始之后,也即已经调用了技能的start方法和init方法之后。
     * @param source 源角色，执行技能的角色。
     * @param skill 执行的技能
     */
    void onSkillStart(Actor source, Skill skill);
     ...
     * @param skill 
     */
    void onSkillEnd(Actor source, Skill skill);
}
